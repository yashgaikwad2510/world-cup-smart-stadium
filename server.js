...
function detectLanguage(text) {
  ...
  if (/\b(hola|d\u00f3nde|qu\u00e9|c\u00f3mo|gracias|por favor|est\u00e1|necesito)\b/.test(lower)) return 'es';
...
function getMockResponse(userMessage) {
  ...
  if (msg.includes('hello') || msg.includes('hi') || msg.includes('hey')) {
    return { type: 'standard', title: null, body: "Hello! Welcome to MetLife Stadium for the FIFA World Cup 2026. \ud83c\udfdf\ufe0f\n\nI am **MetLife Assist**, your GenAI Copilot. I can help you with:\n- \ud83d\udeb6 Navigation & Accessibility paths\n- \ud83d\ude97 Transport updates\n- \ud83c\udf54 Concession stand wait times\n- \ud83d\udea8 Emergency information\n\n*Running in demo mode.*", cards: [], tip: null };
  }
  if (msg.includes('gate') || msg.includes('crowd') || msg.includes('congestion') || msg.includes('density') || msg.includes('shortest')) {
    return { type: 'data_card', title: 'Gate Traffic Status', body: 'Here are the current gate wait times across MetLife Stadium:', cards: [{ label: 'Gate A (North)', value: '35 min wait \u2014 High Traffic', status: 'critical' }, { label: 'Gate B (South)', value: '15 min wait \u2014 Medium Traffic', status: 'warning' }, { label: 'Gate C (East)', value: '5 min wait \u2014 Low Traffic \u267f', status: 'good' }], tip: '\ud83d\udca1 Gate C has the shortest wait. Based on flow patterns, Gate A should ease to ~15 min within 20 minutes.' };
  }
  if (msg.includes('wheelchair') || msg.includes('accessible') || msg.includes('mobility') || msg.includes('ramp') || msg.includes('sensory') || msg.includes('elevator')) {
    return { type: 'navigation', title: 'Accessibility Routes', body: '\u267f **Accessibility is our top priority.**\n\n- **Accessible Entry:** Gate C (East) \u2014 low traffic, 5 min wait\n- **Elevators/Ramps:** Step-free routes near Section 110, 215, 312\n- **Sensory Room:** Located near **Section 212** (Club Level)\n\nStadium staff at Gate C can assist with wheelchair access.', cards: [{ label: 'Gate C (Accessible)', value: '5 min wait', status: 'good' }, { label: 'Sensory Room', value: 'Section 212 \u2014 Club Level', status: 'good' }], tip: 'Ramps and escalators take you directly to the Club Level for Sensory Room access.' };
  }
  if (msg.includes('food') || msg.includes('burger') || msg.includes('taco') || msg.includes('concession') || msg.includes('eat') || msg.includes('nachos')) {
    return { type: 'data_card', title: 'Concession Wait Times', body: 'Here are the current concession lines:', cards: [{ label: 'Sec 224 \u2022 Champions Club Burgers', value: '5 min wait \u2014 Trophy Cheeseburger', status: 'good' }, { label: 'Sec 112 \u2022 Goalpost Tacos & Nachos', value: '12 min wait \u2014 Carne Asada Nachos', status: 'warning' }, { label: 'Sec 318 \u2022 Strikers Hot Dogs & Brew', value: '20 min wait \u2014 Footlong Jersey Dog', status: 'critical' }], tip: '\ud83d\udca1 Section 224 has the shortest line right now!' };
  }
  if (msg.includes('transit') || msg.includes('train') || msg.includes('bus') || msg.includes('rideshare') || msg.includes('uber') || msg.includes('lyft') || msg.includes('transport') || msg.includes('penn')) {
    return { type: 'data_card', title: 'Transportation Status', body: 'Here is the latest transport update:', cards: [{ label: 'NJ Transit Rail (Lot B)', value: 'On Time \u2014 Every 10 min to Penn Station', status: 'good' }, { label: 'Coach USA Bus (Lot A)', value: 'Minor Delays \u2014 Route 3 traffic', status: 'warning' }, { label: 'Rideshare (Lot E)', value: '20-25 min wait \u2014 Surge pricing active', status: 'critical' }], tip: '\ud83d\udca1 NJ Transit Rail from Lot B is your fastest option right now!' };
  }
  if (msg.includes('emergency') || msg.includes('evacuat') || msg.includes('fire') || msg.includes('help') || msg.includes('danger') || msg.includes('medical')) {
    return { type: 'alert', title: '\u26a0\ufe0f Emergency Information', body: '**If you are experiencing an emergency:**\n\n1. **Stay calm** and follow stadium staff directions\n2. **Nearest exits** are illuminated in all sections\n3. **Gate C (East)** is the designated accessible emergency exit\n4. **Medical stations** are located at Sections 110, 215, and 318\n5. **Call stadium security:** Dial extension 911 on any courtesy phone', cards: [{ label: 'Emergency Exit', value: 'Gate C (East) \u2014 Accessible', status: 'critical' }, { label: 'Medical Station', value: 'Sections 110, 215, 318', status: 'warning' }], tip: 'If this is a life-threatening emergency, call 911 immediately.' };
  }

  return { type: 'standard', title: null, body: "I can help you navigate MetLife Stadium! Feel free to ask about:\n\n- \ud83d\udeb6 **Gates & crowd density**\n- \u267f **Accessible pathways & elevators**\n- \ud83d\ude86 **Transit options**\n- \ud83c\udf54 **Concession wait times**\n- \ud83d\udea8 **Emergency information**\n\nWhat can I help you with?", cards: [], tip: null };
}
...
// Stadium data for sidebar widgets
...
app.post('/api/chat/stream', rateLimitMiddleware, async (req, res) => {
  ...
  if (lastMsg) {
    ...
    if (!validation.safe) {
      ...
      res.flushHeaders();
      const errorResponse = { type: 'alert', title: 'Input Blocked', body: `\u26a0\ufe0f ${validation.reason}. Please rephrase your question about MetLife Stadium.`, cards: [], tip: null };
  ...
  try {
    ...
    const fullSystemInstruction = buildSystemInstruction(gatesContext, concessionsContext, transportContext);
    ...
    if (ai) {
      ...
      res.write(`data: ${JSON.stringify({ done: true, structured: mockResponse, detectedLang })}\n\n`);
      res.end();
    }
  } catch (error) {
    console.error('Streaming error:', error);
    const errorResponse = { type: 'alert', title: 'Error', body: 'An error occurred while processing your request. Please try again.', cards: [], tip: null };
    res.write(`data: ${JSON.stringify({ done: true, structured: errorResponse })}\n\n`);
    res.end();
  }
});
  ...
  if (lastMsg) {
    ...
    if (!validation.safe) {
      return res.status(400).json({
        text: JSON.stringify({ type: 'alert', title: 'Input Blocked', body: `\u26a0\ufe0f ${validation.reason}.`, cards: [], tip: null }),
        structured: true
      });
    }
    lastMsg.content = validation.text;
  ...
  try {
    ...
    if (ai) {
      ...
      try {
        ...
        const wrapped = JSON.stringify({ type: 'standard', title: null, body: responseText, cards: [], tip: null });
